nll_hmm <- function(theta, x, N, sdd) {
  Gamma <- diag(N)
  ...
  for (j in 1:N) {
    allprobs[ind, j] <- stats::dgamma(
      x[ind], shape = mu[j]^2 / sigma[j]^2, scale = sigma[j]^2 / mu[j]
    )
  }
...
fit_hmm <- function(data, N, sdd) {
  theta <- c(rep(-2, (N - 1) * N), log(seq(1, 10, length = N)), log(seq(1, 10, length = N)))
  mod <- suppressWarnings(nlm(nll_hmm, theta, x = data, N = N))
  theta <- mod$estimate
  Gamma <- diag(N)
  Gamma[!Gamma] <- exp(theta[1:((N - 1) * N)])
  Gamma <- Gamma / rowSums(Gamma)
  ...
  mu <- exp(theta[(N - 1) * N + 1:N])
  sigma <- exp(theta[(N - 1) * N + (N + 1):(2 * N)])
  list(N = N, Gamma = Gamma, delta = delta, mu = mu, sigma = sigma)
}
...
decode_states <- function(data, model) {
  T <- length(data)
  N <- model$N
  Gamma <- model$Gamma
  delta <- model$delta
